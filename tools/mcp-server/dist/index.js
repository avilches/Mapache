...
async function toolTypecheck() {
    const { output, exitCode } = await runCommand("npx tsc --noEmit", MOBILE_DIR, 60_000);
    if (exitCode === 0) {
...
async function toolTest() {
    const { output, exitCode } = await runCommand("npm test -- --watchAll=false --ci 2>&1 || true", MOBILE_DIR, 120_000);
    const header = exitCode === 0
        ? "✓ Tests passed."
        : `Tests failed (exit ${exitCode}).`;
    return `${header}\n\n${output || "(no output)"}`;
}
async function toolValidateLevels() {
    const scriptPath = path.join(ADMIN_DIR, "validate_levels.py");
    if (!fs.existsSync(scriptPath)) {
        return `Error: validate_levels.py not found at ${scriptPath}`;
    }
    const { output, exitCode } = await runCommand(`python3 "${scriptPath}"`, PROJECT_ROOT, 30_000);
    const header = exitCode === 0 ? "✓ Packs valid." : `Validation failed (exit ${exitCode}).`;
    return `${header}\n\n${output || "(no output)"}`;
}
async function toolExpoStart() {
    if (expoProcess !== null) {
    ...
    return (`Expo started (PID ${proc.pid}).\nLog file: ${logFile}\n\n` +
        `First output:\n${preview || "(none yet)"}`);
}
async function toolExpoStop() {
    if (expoProcess === null) {
...
async function toolExpoLogs(lines) {
    if (expoProcess === null) {
        return "No expo process is currently running.";
    }
...
// ─── Server setup ─────────────────────────────────────────────────────────────
const server = new Server({ name: "mapache-dev", version: "1.0.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "typecheck",
            description: "Run `npx tsc --noEmit` in mobile/ and return TypeScript errors.",
            inputSchema: {
                type: "object",
                properties: {},
                required: [],
        ...
        {
            name: "test",
            description: "Run `npm test` in mobile/ and return pass/fail status and output.",
            inputSchema: {
                type: "object",
                properties: {},
                required: [],
        ...
        },
        {
            ...
            description: "Run `python3 admin/validate_levels.py` and return validation results.",
            inputSchema: {
                type: "object",
                properties: {},
                required: [],
        ...
        {
            name: "expo_start",
            description: "Start `npx expo start --non-interactive` in background in mobile/. Returns PID and initial log output.",
            inputSchema: {
                type: "object",
                properties: {},
                required: [],
